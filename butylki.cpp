...
{
    if (pb->hp <= 0) {
        if (txMessageBox ("Бутылка, ты окончательно умер?", "Соглашайся, второго шанса не будет", MB_YESNO) == IDYES) {
    ...
    if (vrag->hp <= 0) {
        if (txMessageBox ("Враг, ты окончательно умер?", "Соглашайся, второго шанса не будет", MB_YESNO) == IDYES) {
...
void ranen(Butylka* pb, Butylka* vrag, Bullet* bulletButylki, Bullet* bulletVraga)
{
     for (int i = 0; i < OBJOM_MAGAZINA; i++) {
            ...
            if (vrag->hp > 0 &&
                ...
                bulletButylki[i].y >= vrag->y) {//ïîïàäàíèå {
